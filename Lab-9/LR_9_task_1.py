...
# Парсер аргументів для вибору типу класифікатора
...
if __name__=='__main__':
    # Отримання аргументів (запуск через термінал)
    # Приклад: python LR_9_task_1.py --classifier-type rf
    try:
        args = build_arg_parser().parse_args()
        classifier_type = args.classifier_type
    except:
        # Якщо запущено без аргументів у IDE, ставимо значення за замовчуванням
        classifier_type = 'erf' 
        print(f"Аргументи не надано. Використовується за замовчуванням: {classifier_type}")
    ...
    # Розбиття на класи для візуалізації вхідних даних
    ...
    # 2. Розбивка на навчання та тест
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.25, random_state=5)

    # 3. Конструювання класифікатора
    params = {'n_estimators': 100, 'max_depth': 4, 'random_state': 0}
    
    if classifier_type == 'rf':
        classifier = RandomForestClassifier(**params)
        print("Обрано: Random Forest")
    else:
        classifier = ExtraTreesClassifier(**params)
        print("Обрано: Extremely Random Forest")

    classifier.fit(X_train, y_train)
    visualize_classifier(classifier, X_train, y_train, f'Training dataset ({classifier_type})')

    # 4. Перевірка на тестових даних
    ...
    # 6. Оцінка довірливості (Confidence Measure)
